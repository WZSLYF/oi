...
*   CreatedTime: 2022年06月18日 星期六 19时24分44秒
*   Describe: 
*   
**************************************************************************/
...
int main()
{
    ...
    //单调队列 + 二分
    int tot = 0;
    while (std::cin >> dat[++tot]);
    tot--;
    int len = 1;
    f[1] = dat[1];
    Rep(i, 2, tot) {
        if (dat[i] <= f[len]) f[++len] = dat[i];
        else {
            int pos = std::upper_bound(f + 1, f + len + 1, dat[i], std::greater<int>()) - f;
            f[pos] = dat[i];
        }
    }
    ...
    Rep(i, 2, tot) {
        if (dat[i] > f[len]) f[++len] = dat[i];
            ...
            int pos = std::lower_bound(f + 1, f + len + 1, dat[i]) - f;
            f[pos] = dat[i];
        }
    }
    std::cout << len << std::endl;
