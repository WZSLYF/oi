...
*   CreatedTime: 2022年05月01日 星期日 22时40分06秒
*   Describe: 
*   
**************************************************************************/
...
int main()
{
    ...
    io.read(n);
    vc<matrix> dat(n);
    for (auto &x : dat) io.read(x.x1, x.y1, x.x2, x.y2), x.x2 = x.x1 + x.x2, x.y2 = x.y1 + x.y2;
    int xx, yy;
    io.read(xx, yy);
    int ans = -1;
    Resp(i, n - 1, 0) if (dat[i].x1 <= xx && xx <= dat[i].x2 && dat[i].y1 <= yy && yy <= dat[i].y2) {
        ans = i + 1;
        break;
    }
    io.write(ans, '\n');
